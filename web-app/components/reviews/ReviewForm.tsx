    ...
    <div className="flex items-center gap-2">
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <svg
              className={`w-6 h-6 transition-colors ${star <= active ? "text-amber-400" : "text-gray-200"}`}
      ...
      <span className={`text-xs font-medium w-14 ${active ? "text-amber-500" : "text-transparent"}`}>
        {LABELS[active]}
      </span>
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 pt-5 pb-4 border-b border-gray-100">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="text-base font-extrabold text-gray-900">{professorName}</h2>
            </div>
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto max-h-[60vh]">
          ...
          <div className="space-y-3">
            {CHARACTERISTICS.map(({ key, label, desc }) => (
              <div key={key} className="space-y-0.5">
                <p className="text-xs font-bold text-gray-900 text-left">{label}</p>
          ...
          {overallPreview && (
            <div className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-xl px-3 py-2">
              <span className="text-xs font-semibold text-blue-700">Overall Rating</span>
              <span className="text-base font-black text-blue-700">{overallPreview} <span className="text-xs font-normal">/ 5</span></span>
            </div>
          )}
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
              style={{ color: "#111827" }}
            />
          </div>
        ...
        {/* Footer — always visible, not inside scroll area */}
        <div className="px-5 pb-5 pt-3 border-t border-gray-100 space-y-3">
          {/* Anonymous toggle — pinned above submit */}
          <div className="flex items-center justify-between">
            <div className="text-left">
              <p className="text-xs font-bold text-gray-900">Post anonymously</p>
              <p className="text-xs text-gray-600">Your name won't appear on the review</p>
            </div>
            <button
              type="button"
              onClick={() => setIsAnonymous(!isAnonymous)}
              className={`relative w-9 h-5 rounded-full transition-colors flex-shrink-0 ml-4 ${isAnonymous ? "bg-blue-600" : "bg-gray-300"}`}
            >
              <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${isAnonymous ? "translate-x-4" : "translate-x-0"}`} />
            </button>
          </div>
